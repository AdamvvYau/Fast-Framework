...
namespace Fast.Framework.Abstract
{
    ...
    public abstract class InsertBuilder : ISqlBuilder
    {
            ...
            for (int i = 0; i < batchCount; i++)
            {
                ...
                if (IsDictionaryList)
                {
                    foreach (Dictionary<string, object> item in list.Cast<Dictionary<string, object>>())
                    {
                        ...
                        {
                            var value = item[columnInfo.ColumnName];
                            if (columnInfo.IsJson && value != null)
                            {
                                value = Json.Serialize(value);
                            }
                            var parameter = new FastParameter($"{columnInfo.ColumnName}_{parameIndex}", value);
                            parameter.GroupId = i;
                ...
                else
                {
                    foreach (var item in list)
                    {
                        ...
                        columnInfosList.Add(columnInfoList);
                        ComputedValues.AddRange(columnInfoList.ComputedValues(dbParameters));
                    }
                }
        ...
        public virtual InsertBuilder Clone()
        {
            ...
            insertBuilder.ComputedValues.AddRange(this.ComputedValues);
